...
export class PictureStepComponent {
  ...
  //pictures = input.required<Array<NewJobPicture>>();
  ...
  @Output()
  picturesChange = new EventEmitter<Array<NewJobPicture>>();
  ...
  onUploadNewPicture(target: EventTarget | null) {
    ...
    if(picturesFileList !== null) {
      for(let i = 0 ; i < picturesFileList.length; i++) {
        ...
        if (picture !== null) {
          const displayPicture: NewJobPicture = {
            file: picture,
  ...
  onTrashPicture(pictureToDelete: NewJobPicture) {
